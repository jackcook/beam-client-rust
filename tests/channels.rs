...
fn test_channel_with_token() {
    let beam = Beam::new();
    let res = beam.channels_routes().get_channel_with_token(String::from(CHANNEL_TOKEN));
...
fn test_rising_channels() {
    let beam = Beam::new();
    let res = beam.channels_routes().get_channels(ChannelsRequestType::Rising, 0);
...
fn test_fresh_channels() {
    let beam = Beam::new();
    let res = beam.channels_routes().get_channels(ChannelsRequestType::Fresh, 0);
